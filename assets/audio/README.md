...
`test/sfx/` の11点をそのまま採用。互いに代替関係になく、すべて別々の用途を持つため。
内訳と使用箇所は下の「効果音」の表を参照。

### 編集時の注意

**トリミングは Resolve 側で行い、音源ファイルは加工しない**（非破壊編集を保つ）。
特に次の3つは扱いを誤ると成立しない。

| ファイル | 注意点 |
|---|---|
| `id-card-drop.mp3` | 冒頭に約1秒の空白あり。**イン点をアタックまで詰めてから**配置する。クリップ端をカット点に合わせると音が1秒遅れる |
| `camera-shutter-burst.mp3` | 同上。カット11の同期発光の拍に、ファイル先頭ではなく**音の立ち上がり**を合わせる |
| `heartbeat-low.mp3` | 11秒〜30秒過ぎまで約20秒ループさせる。前後の空白を詰め、**継ぎ目で拍が途切れない**長さに整える |
