...
P1Cは、アプリ内AIチャットをWorkers経由でOpenAIへ中継し、チームごとに複数スレッドの会話を保存する。本ドキュメントは実装が進むごとに更新する。現在の内容はチャット骨格（スレッド・メッセージ・OpenAI adapter）と送信前PIIゲートまでを反映する。D1ログと障害耐性の分類は後続のPRで追記する。
...
リクエスト件数の利用制限は2026-09-04に導入済みである。チーム単位のレート制限（1分あたり`CHAT_RATE_LIMIT_PER_MINUTE`通、既定20。送信前PII拒否の経路も同じ枠を消費する）と、スレッド作成の上限（手動25件・ステージ8件を独立に数える）が入っている。設定値は`docs/development-harness.md`の「公開APIガードの環境変数」を正とする。

**トークン予算は未導入である**——件数は数えているが、1通あたりの入出力トークン量は制限していない。トークン予算、D1へのAIログ保存、障害の分類とフォールバック、KV・R2、Stage 4のインシデント状態機械（黒塗り罰ゲーム・AI使用ロック・回答期限）は後続PRで追加する。
