...
- **返答には［グリッドへ送る］がある**（Issue #81・表の上＝導入文の直後。20〜30行の表の下に置くと
  隠れて見つからないため）。押すと同じ吹き出しの `<pre class="tsv">` を `parseTable()` で読み、
  グリッドが全セル空なら即時反映（`.grid.landed` で一瞬光る）、1文字でも入っていれば貼り付けと同じ
  確認バー（`#paste-bar`）で確認させる。`view !== "s2"` なら何もしない。従来のコピー＆貼り付け経路
  （列選択コピー・［作業エリアに追加］・貼り付け確認バー）はそのまま残してある。
