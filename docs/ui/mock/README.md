...
`sendAI()`（1662）は `view === "s5"` のとき `S4_PII` の検査を素通りし、自分の吹き出しを出したあと
`s5Generate()`（3396）へ渡す——Stage 3・4 の「罠 → 台本応答」ではなく「プロンプト → 候補画像」という
別の分岐にしてある。

- **モックは gpt-image-1 を呼ばない。** `S5_POSTERS`（1464）の `tag` 正規表現とプロンプト文字列を
  照合し、最初に一致した候補を返す。どれにも一致しなければ `S5_POSTER_DEFAULT`（1472）。
  生成待ちは `S5_GEN_MS`（1520・仮値2.5秒）の `later()` で、実際にAPIを叩いているように見せる。
...
- **`runStage5Verdict()`（3488）に「まだ足りません」の分岐は無い。** 候補が選ばれていれば
...
- **`s5PickCandidate()`（3433）で選んだ候補だけが `#s5-cand`（`s5DrawCandidate()`3440）に反映される。**
