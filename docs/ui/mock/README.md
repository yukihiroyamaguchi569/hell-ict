...
失敗したら「サーバに接続できません。」と［再試行］を出し、**scriptedへ黙って落とさない**。
短いタイムアウトで静かにscriptedへ落ちると、当日それに気づくのは「入室したのに他のチームが帯に出ない」と
言われた後になる。副作用として、**APIを持たない静的サーバ（`python -m http.server` など）で開くと
入室できなくなる**——scriptedで触りたいときは `file://` で直接開くこと。
