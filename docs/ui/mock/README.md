...
- **ヘッダーの A−／A＋ で文字サイズを3段階（標準／大／特大）に切り替える**（2026-09-05追加・issue #87）。
  ...
  ——メール本文（`.reader` `.legacy`）・課題文（`.stage-title` `.brief`）・提出フォーム・判定文・AIの吹き出しと
  入力欄・院内連絡先（`.phs`）・添付ビューア本文——に限る。ヘッダー・リーダーボード帯・ペインの固定幅・
  ボタンの寸法・提出グリッドの表には掛けない（配置と横幅を段階で動かさないため）。状態は
  `localStorage.hellFontStep` に残す。
