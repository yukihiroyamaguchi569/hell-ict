...
| `TEAM_CODES` | 6桁なら任意のコードで入室できる（ローカル開発とE2Eを壊さないための意図的なfail-open） | 本番。事前配布した6桁コードをカンマ区切りで列挙する。列挙外のコードは入室・チーム操作・リーダーボード購読・進捗記録のすべてで404になり、Durable ObjectもD1の行も作らない。**空文字や`,`だけを設定した場合は「設定し損ね」とみなして全コードを拒否する（fail-closed）**。要素は1つずつ6桁数字として検証し、1つでも不正なら全体を無効（`guards.teamCodes` が `"invalid"`）として全コードを拒否する。**空要素（`100001,,100002` や末尾カンマ）も不正として扱う**——落として済ませると区切りの打ち間違いに気づけず、`teamCodesCount` を配布数と突き合わせる確認が効かなくなる。区切りは半角カンマのほか全角カンマ「，」と読点「、」も受ける（配布表から手で貼る運用のため）。重複は除去する |
...
4. デプロイ後、`GET /api/health`の`guards`で設定が効いているか確認する。`{"status":"ok","guards":{"teamCodes":true,"teamCodesCount":7,"allowedOrigins":false,"chatRateLimitPerMinute":20}}`のように返るので、次の2つを必ず見る。値そのものは伏せてあるので、この応答から配布コードや許可オリジンは漏れない。
   - **`teamCodes`が`true`であること。** `false`なら`TEAM_CODES`の設定漏れで、6桁なら誰でも入れる状態のまま本番を迎えることになる。`"invalid"`なら値が壊れている（6桁数字でない要素が混ざっている）状態で、**全チームが404で入室できない**——書き損じをここで捕まえる。
   - **`teamCodesCount`が配布チーム数と一致すること。** 一致しなければ、区切りの書き損じか重複がある。重複は除去して数えるので、7チームへ配ったのに6と出たらどこかで同じコードを2度書いている。
