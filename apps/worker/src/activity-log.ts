 ...
 *
 * 名前を`_v2`にしたうえで、旧名を先にDROPする。CREATE INDEX IF NOT EXISTSは
 * 「同じ名前が既にある」だけで何もしないので、キー構成を変えても既存のD1には
 * 反映されない——旧定義のまま残り、狭いキーで本物のイベントを捨て続ける。
 * 定義を変えるときは名前も変え、旧名を落としてから作り直す。
 */
